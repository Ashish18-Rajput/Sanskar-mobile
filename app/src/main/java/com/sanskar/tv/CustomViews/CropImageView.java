...
public class CropImageView extends androidx.appcompat.widget.AppCompatImageView {
	...
	private boolean disableFrame=false;
	...
	/*
	 * private void moveHandleLT(float diffX, float diffY) { if (mCropMode ==
	 * CropMode.RATIO_FREE) { mFrameRect.left += diffX; mFrameRect.top += diffY;
	 * if (isWidthTooSmall()) { float offsetX = mMinFrameSize - getFrameW();
	 * mFrameRect.left -= offsetX; } if (isHeightTooSmall()) { float offsetY =
	 * mMinFrameSize - getFrameH(); mFrameRect.top -= offsetY; }
	 * checkScaleBounds(); } else { float dx = diffX; float dy = diffX *
	 * getRatioY() / getRatioX(); mFrameRect.left += dx; mFrameRect.top += dy;
	 * if (isWidthTooSmall()) { float offsetX = mMinFrameSize - getFrameW();
	 * mFrameRect.left -= offsetX; float offsetY = offsetX * getRatioY() /
	 * getRatioX(); mFrameRect.top -= offsetY; } if (isHeightTooSmall()) { float
	 * offsetY = mMinFrameSize - getFrameH(); mFrameRect.top -= offsetY; float
	 * offsetX = offsetY * getRatioX() / getRatioY(); mFrameRect.left -=
	 * offsetX; } float ox, oy; if (!isInsideHorizontal(mFrameRect.left)) { ox =
	 * mImageRect.left - mFrameRect.left; mFrameRect.left += ox; oy = ox *
	 * getRatioY() / getRatioX(); mFrameRect.top += oy; } if
	 * (!isInsideVertical(mFrameRect.top)) { oy = mImageRect.top -
	 * mFrameRect.top; mFrameRect.top += oy; ox = oy * getRatioX() /
	 * getRatioY(); mFrameRect.left += ox; } } }
	 */
	...
	public void rotateImage(RotateDegrees degrees) {
		...
		Bitmap rotated=null;
		try {
			 rotated = Bitmap.createBitmap(source, 0, 0, width,height, matrix, true);
		} catch (OutOfMemoryError e) {
			rotated= Bitmap.createBitmap(source,0,0, 3*width/2,3*height/2, matrix, true);
		}
		if(rotated!=null){
	...
	public Bitmap getCroppedBitmap() {
		...
		mIsInitialized=false;
		try{
		 cropped = Bitmap.createBitmap(source, x, y, w, h, null, false);
		if (mCropMode != CropMode.CIRCLE)
			return cropped;
		}
		catch(Throwable e){
	 ...
	 * <p/>
	 * This method always returns rect image. (If you need a square image with
	 * CropMode.CIRCLE, you can use this method.)
	 * 
	 * @return cropped image bitmap
	 */
	 ...
	 * 
	 * @param minDp
	 *            crop frame minimum size in density-independent pixels
	 */
	 ...
	 * 
	 * @param minPx
	 *            crop frame minimum size in pixels
	 */
	 ...
	 * 
	 * @param paddingDp
	 *            crop frame handle touch padding(touch area) in
	 ...
	 * 
	 * @param weightDp
	 *            frame stroke weight in density-independent pixels.
	 */
	 ...
	 * 
	 * @param weightDp
	 *            guideline stroke weight in density-independent pixels.
	 */
